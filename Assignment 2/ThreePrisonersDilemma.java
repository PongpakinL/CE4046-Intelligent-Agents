public class ThreePrisonersDilemma {
	...
	class AdaptiveT4TPlayer extends Player {
		int defectCount = 0;
		...
		int selectAction(int n, int[] myHistory, int[] oppHistory1, int[] oppHistory2) {
			...
			// Count the number of defections and cooperations in the previous round
			int myLastMove = myHistory[n-1];
			int oppLastMove1 = oppHistory1[n-1];
			int oppLastMove2 = oppHistory2[n-1];
			int numCooperations = (myLastMove == 0 ? 1 : 0) + (oppLastMove1 == 0 ? 1 : 0) + (oppLastMove2 == 0 ? 1 : 0);
			int numDefections = 3 - numCooperations;
	
			// Update the defect and cooperate counts
			if (myLastMove == 1) {
				defectCount++;
			} else {
				cooperateCount++;
			...
			// Check if the opponent is defecting more than cooperating
			boolean opponentDefecting = (numDefections > numCooperations);
	
			// Adjust strategy based on performance so far
			if (defectCount > cooperateCount) {
				// If we've been defecting more than cooperating, become more forgiving
				return (opponentDefecting ? 1 : 0);
			} else {
				// Otherwise, continue using Tit-for-Tat
				return oppLastMove1;
			}
		}
	}
	...
	class GrimTriggerPlayer extends Player {
		...
		int selectAction(int n, int[] myHistory, int[] oppHistory1, int[] oppHistory2) {
			if (!triggered) {
				// Cooperate until the other player defects
				if (n == 0 || oppHistory1[n-1] == 0 && oppHistory2[n-1] == 0) {
					return 0;
				} else {
					triggered = true;
					return 1;
...
//	This is a helper function needed by scoresOfMatch.
	...
	Player makePlayer(int which) {
		...
		case 6: return new Lumlertluksanachai_Pongpakin_Player();
		case 7: return new AdaptiveT4TPlayer();
